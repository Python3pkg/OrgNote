...
class OrgNote(object):
    ...
    def gen_tag_href(self,name=""):
        if name not in list(self.menus_map.keys()):
    ...
    def contain_prefix(self,tags=[],name="",header_title=""):
        ...
        elif header_title in self.__pagenames__:
    ...
    def gen_timetag_list(self,public=True):
        ...
        for num,link in enumerate(self.notes):
            ...
            if yyyymm not in self.timetags:self.timetags[yyyymm] = list()
    ...
    def sidebar_date(self):
        ...
        """
        tot = 0
        for key in sorted(list(self.timetags.keys()),reverse=True):
    ...
    def gen_category(self,link=""):
        ...
        if keywords_list and 'content' in keywords_list[0].attrs:
